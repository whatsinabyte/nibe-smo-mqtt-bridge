...
| `family` | — | string | Set to `air_water` or `water_water` when a menu/submenu applies to only one hardware family — e.g. the GP1 heating medium pump and brine-circuit submenus under 7.1.2, which only exist on ground-source models. The bridge detects the connected controller's family from its reported product model (see `_detect_controller_family` in `nibe_lovelace.py`) and drops any menu/submenu whose `family` doesn't match before rendering the dashboard — the tab or section simply doesn't appear, no "not applicable" placeholder shown. Omit the field for anything that applies to every installation (the vast majority of menus — shared accessories like ERS, ACS, EME20, Pool, etc.). This only affects what appears in the dashboard; it does not change which points/entities get enabled. |
| `split_submenus` | — | boolean | Set to `true` on a menu with enough content that flattening every submenu into one dashboard view/tab makes that tab unusably long. Instead of one view for the whole menu, the menu itself becomes a small "hub" view (its own description/warning/note/tip, plus a list of links to each immediate submenu) and each immediate submenu becomes its own separate, fully-rendered view — see `_build_view_specs` in `nibe_lovelace.py`. Currently set on menus 1, 4 and 7 (each large across several balanced-sized submenus), plus submenu 3.1 specifically rather than menu 3 itself (menu 3's own bulk sits almost entirely inside 3.1, with its other seven submenus tiny or empty — splitting menu 3 too would force those into an extra click each for no benefit). The flag isn't limited to top-level menus: a submenu buried inside an otherwise-unsplit parent can carry it, and gets pulled out into its own view while the rest of that parent's tree still flattens normally — see `_find_split_descendants`. The one spot where it would have appeared inline gets a short "jump to its own view" card instead (`_build_jump_out_card`), the same way a hub links to its own children. This also changes what counts as "the same page" for `_linkify_menu_refs`: a cross-reference between two submenus of a split menu (e.g. a "menu 7.2.4" mention written inside menu 7.1's text) becomes a real, clickable link instead of same-page dead text, since the two submenus are now genuinely different views. Omit for a menu with too little content to be worth the extra click a split adds. |
...
Do not delete one side of a duplicate without establishing which menu the controller really shows it in. Deleting a legitimate one silently removes a control from a dashboard section where users expect it. The test that does justify removal is a register whose own title scopes it to something narrower than the menu holding it: point 14314 is titled "Modbus TCP/IP Ext. Available power **(Solar PV)**" and belongs with its two siblings in 7.5.12, the PV-inverter menu, so the copy that had been sitting in the general 7.5.9 Modbus menu was removed.
...
**Menus known to exist but deliberately not represented.** These are named in the firmware changelog or an installer manual, but nothing available says what they contain, so adding them would mean inventing their rows: `3.1.1` and `3.1.4` (Operating info, referenced only as "improved info in menu X"), `3.1.7` (referenced once for an S1155-25 dew temperature fix), `1.3.30` Unassigned units and `7.5.6` Inverter replacement (both listing or guide screens with no settings), `4.11.5` (manual PVT defrosting, an action), `5.1.1` (myUplink Premium information), `7.1.2.9` (brine pump display for a GSHP docking), and `7.1.7.6` (an S735C extract-air selection). Menu `4.30.1` is excluded for a different reason: it belongs to VVM S330 and SVM S332, which this bridge does not support.
