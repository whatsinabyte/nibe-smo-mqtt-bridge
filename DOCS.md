...
| `language` | Query language for the Nibe REST API. Dropdown listing every language the SMO S40's own "Language" select entity supports, all of them verified to translate against a live controller. None of them fail — what varies is how complete and how natural NIBE's own translations are, which a native speaker will notice before anyone else. `auto` detects Home Assistant's own configured language; any other value overrides auto-detection. An unrecognised code silently falls back to English, never an error. Does not translate the "Nibe Menus" Lovelace dashboard, which is static English content. Restart required. | `auto` |
