...
|---|---|---|
| `nibe_host` | IP address of your controller | `192.168.1.100` |
...
| `log_level` | Log verbosity. Does not affect which entities are exposed — see `debug_mode` below for diagnostic entities. Restart required. | `info` |
| `debug_mode` | Unlocks diagnostic entities on the Management device (Flush Dynamic Map, Run Test Suite, Test Suite Result, Test API Connection, Connectivity Check Result) — a separate setting from `log_level`. Restart required. | `false` |
...
| `advanced` | Monitoring plus diagnostic and tuning registers (PV panel mode, compressor tuning, periodic hot water increase, manual power settings) |
...
> ℹ️ **Entity names follow the `language` setting, not the controller's own on-screen language.** Set `language` to translate entity titles/descriptions (see [Configuration](#configuration)); leave it at `auto` (default) or pick `en` for English. Titles/descriptions come entirely from the controller's own REST API, translated via its `Accept-Language` header. Separately, the app ships its own small hand-maintained translation table for a handful of enum-style state *values* (e.g. "Auto", "Heating", "Cooling") that the controller doesn't translate on its own — this only covers 12 languages (cs, da, de, en, es, fi, fr, it, nl, no, pl, sv); any other `language` falls back to English for these specific values, even though titles/descriptions still translate via the API as normal. The dropdown lists every language the controller's own "Language" select entity supports, hand-verified against real firmware since NIBE documents none of this. Only nl, de, and sv have been confirmed against a live controller; the rest are expected to work the same way but are untested — none is known to fail outright, and an unrecognised code falls back to English silently, never an error.
...
The following entities are only visible when `debug_mode` is enabled (a separate setting from `log_level` — see [Configuration](#configuration) above):

| Entity | Type | Description |
|---|---|---|
| Flush Dynamic Map | Button | Clear all learned dynamic point relationships |
| Run Test Suite | Button | Trigger the built-in test suite — takes roughly 30 minutes on an ODROID-M1 for the current suite size; the sensor stays `running` until it finishes |
...
Snapshots are stored in `/data/snapshots.json` inside the app's data volume. They survive app restarts, app updates/rebuilds, host reboots, and MQTT broker restarts. They are included in HA backups. The app itself never deletes this file — not even when **Remove frontend** is enabled, which only clears the dashboard, card file, and retained MQTT messages.

The one thing that does remove snapshots is **fully uninstalling the app** — Home Assistant's Supervisor deletes the entire `/data` volume as part of removing an app, before the app's own code runs again, so this happens regardless of any app setting. If you need to uninstall and reinstall (for example, to resolve a version-mismatch issue after local development), note the contents of your snapshots first if you want to recreate them afterward — an uninstall/reinstall is not the same as a normal restart or update, and snapshots do not survive it.
...
**Restart required for configuration changes** — changing `nibe_host`, credentials, `poll_interval`, `mode`, `mode_switch_behavior`, `log_level`, `debug_mode`, or `language` requires an app restart. There is no live reconfiguration path.

**Entity name translation covers the API, not the Lovelace dashboard, and its quality is mostly NIBE's, not this bridge's** — the `language` setting translates entity titles/descriptions as reported by the controller's own REST API via its `Accept-Language` header, independent of whatever display language the controller's own on-screen UI is set to — those are two separate translation layers on Nibe's side. It does **not** translate the "Nibe Menus" Lovelace dashboard: that dashboard's section headings and labels are static content defined in the app itself, not sourced from the API, and remain in English regardless of `language`. The dropdown's options mirror the controller's own hand-verified "Language" select entity, not a REST API capability list NIBE has never published — NIBE documents no such list. Only nl, de, and sv have actually been verified against a real controller by requesting them and checking what came back translated; the other listed languages are expected to behave the same way but are untested. None is known to fail outright. An unsupported value simply falls back to English with no warning or error, matching the behaviour of `language: auto` when Home Assistant's own language isn't one the controller recognises either. Translation coverage at the individual title/description level is entirely NIBE's own responsibility — the bridge only ever forwards whatever text the controller's REST API returns for a given point and language. In practice this means the result may be less than a native speaker would call complete or correct, and occasionally a point returns English text under a non-English `language` even while every point around it translates (observed, for example, on point 24492 under `de`). This is a gap in NIBE's own translation catalog for that specific point under whichever language was requested — it is safe to assume NIBE is responsible for every point's translation in every language, and any point that comes back untranslated is a firmware-side omission the bridge cannot detect or work around. In every one of these cases — an unsupported `language` value, a controller that doesn't recognise the requested language at all, or a single point missing a translation while others succeed — English is always what comes back. This is the controller's own fallback, not a special case the bridge has to implement or detect: whatever isn't translated stays in, or reverts to, English by default.

One layer of translation *is* the bridge's own: a handful of enum-style state values (e.g. "Auto", "Heating", "Cooling", "Manual") that the controller's own API doesn't localize on its own are translated via a small hand-maintained table in `translations/<lang>.yaml`, covering 12 languages (cs, da, de, en, es, fi, fr, it, nl, no, pl, sv). Any other `language` value falls back to English for just these specific values, even while titles/descriptions still translate normally via the API.

**Intermittent REST API connection drops on some controllers** — some users have observed the controller stop responding to REST API requests after a period (TCP `SYN` sent, no `SYN,ACK` returned), even though the network path itself is healthy (ARP, cabling, switch port, and firewall rules all check out) and a parallel Modbus TCP connection stays stable throughout. In at least one confirmed case, the actual cause was **another Home Assistant integration port-scanning the local network** — specifically the built-in **nmap Tracker** device-presence integration — hitting the controller's limited embedded TCP stack hard enough to make it stop accepting new connections. Other network-discovery integrations (e.g. UniFi's device tracker) that periodically scan the local subnet could plausibly cause the same thing, since it's the scan traffic itself, not the bridge's own polling, that overwhelms the device. If you use nmap Tracker, UniFi, or a similar scanning integration, try excluding the controller's IP address from its scan range — one user's connection went from failing within minutes to stable for hours after doing exactly this. If that doesn't apply to your setup, check that your controller is on the latest firmware regardless (Nibe has shipped several REST API stability fixes since its introduction in 4.4.7), and consider placing a lightweight reverse proxy between the bridge and the controller's REST API as a fallback — this masked the symptom for one user even before the actual cause was identified. See [discussion #2](https://github.com/whatsinabyte/nibe-smo-mqtt-bridge/discussions/2) for the full diagnostic trail.
