...
**Key invariants:**
- All writes to the controller are serialised through the single-worker write executor. This prevents concurrent PATCH requests from racing. A write's learning-detection phase (see `_run_learning_detection`) can itself occupy that single worker for up to `_POST_WRITE_SCAN_S` (90s) by sleeping in a loop rather than returning immediately, so a write submitted while another is mid-detection doesn't start until it finishes — flipping several switches in quick succession can stack this additively. `_submit_write` logs this explicitly, reporting how many unfinished writes are ahead of the one being submitted. Depth is counted at submit time via `_write_queue_depth` rather than inferred from whether a detection is currently running: that state is only established after the preceding write's PATCH returns, so a burst of switches flipped together all submit before it exists and would each look unqueued. The log deliberately reports queue depth rather than an estimated wait, since whether each queued write opens a detection window at all isn't known until it runs.

Two consequences of that queue follow. `_STALE_WRITE_AGE_S` ages a pending write from when the executor **picked it up** (`started_at`), never from when the command arrived — a write can sit queued arbitrarily long with nothing wrong, and evicting it would drop the pending-write guard and republish the pre-write value, flipping the HA entity back before the write had even been attempted. A write still in the queue is therefore never stale; only one the worker started and never finished is.
- MQTT callbacks never block — heavy work (write commands, management actions) is always dispatched to an executor.
- **No exception may escape an MQTT callback.** The client uses paho's default `suppress_exceptions=False`, so anything propagating out of a callback leaves the network thread's read loop and permanently kills it. That thread is also paho's reconnect loop, so the failure is terminal in both directions: no further commands are delivered and no later reconnect is attempted, leaving a bridge that looks alive but silently ignores Home Assistant until restarted by hand. Every callback — message handlers, `on_connect`, `on_disconnect` — therefore wraps its body in a broad `except Exception` that logs and returns. Note this covers the part that runs *on the callback thread*, which is easy to miss when a handler dispatches its real work to an executor: `ManagementCommandHandler`'s handlers wrapped the closure they hand to `_submit` but not their own `message.payload.decode()` (raises on a malformed payload) or the `_submit` call itself (raises `RuntimeError` once the executor is shut down). Those are guarded centrally in `_sub()`, which wraps every handler as it is registered — including in the record replayed on reconnect — so a handler added later cannot forget.
...
- `_shutdown(...)` — drains executors with a 35-second hard timeout, publishes offline status, optionally clears retained MQTT topics (`NIBE_REMOVE_FRONTEND=1`). That 35s budget is *shared* across the write, management and test executors, so anything that blocks one of them uninterruptibly starves the rest: shutdown must first signal every long in-flight wait to stop. Three exist, and all are signalled before the drain begins — `abort_test_suite()` kills an in-flight test subprocess; `EntityManager.begin_shutdown()` breaks the learning-detection wait, which would otherwise hold the single write worker for the remainder of a 90s window; and that in turn calls `NibeApiClient.begin_shutdown()`, after which no new request is started and a pending retry is abandoned. Without the last one, a single logical request against an unresponsive controller runs two 30s socket timeouts with a backoff between them (~62s) while holding the client's serializing lock, so a second caller waits that long again before its own request even begins. Executor threads are non-daemon, so a wait that outlives the budget also blocks interpreter exit via Python's own atexit join, leaving the container to be SIGKILLed mid-shutdown rather than exiting cleanly.
...
**Durability of `/data` fallback files:** every one of these files (`applied_mode`, `wanted_points.json`, `snapshots.json`, `dynamic_point_map.json`) is read back with its parse failure swallowed as "no data", so a half-written file doesn't surface as an error — it silently discards whatever it held. They are therefore always written via a uniquely-named sibling temp file plus `os.replace` (`_atomic_write_text`, and `DynamicPointMap.to_file`), never by truncating the destination: `open(path, "w")` empties the file before the new contents land, and being killed in that gap is a real scenario, since the shutdown drain budget can exceed a supervisor's stop grace period. This is what makes the write-ahead ordering used by `_persist_applied_mode` and `_persist_wanted_points` (file first, then the retained MQTT topic) actually hold — the file can only ever be one whole version behind, never empty. The temp name must be unique per call rather than a fixed `path + ".tmp"`: none of the callers hold a common lock and they are reachable from the MQTT command thread, the management executor, the registry watcher thread and the poll thread simultaneously, so a shared temp path lets concurrent writers truncate each other's half-written file and rename the mixture into place — measured at roughly half of concurrent runs, alongside spurious `FileNotFoundError`s from one writer's `os.replace` consuming another's temp file.
...
**`VALUE_MAPPINGS` translation:** `VALUE_MAPPINGS` itself is hand-maintained, English-only value→label tables for registers the firmware's own `description` string doesn't enumerate cleanly enough for `parse_description_mapping()` to use — entirely separate from the `language` config option's `Accept-Language` header, which only affects labels the *firmware* returns, not this file's own hardcoded ones. Since `nibe_entity_detection.py` must stay pure, translating these labels happens one layer up: `generate_nibe_mqtt.py`'s `main()` loads `translations/<lang>.yaml`'s `value_mappings:` section once at startup via `_load_value_mapping_translations()` (`nibe_entity_manager.py`) and passes the resulting `{english_label: translated_label}` dict into both `EntityManager` and `MqttDiscoveryPublisher`/`build_select_config()` (`nibe_discovery_config.py`), which look up each `VALUE_MAPPINGS` label through it (falling through to the original English label on any miss) wherever it's published or parsed back from a write — `sensor` state, and `select`'s state, options list, and command round-trip. Only the twelve languages with an existing `translations/<lang>.yaml` file get a `value_mappings:` section at all; any other configured language falls back to the English labels, the same fail-safe behavior as an unsupported REST API language code.

**Description and range can contradict each other.** A register's `description` may enumerate more options than its declared `minValue`/`maxValue` admits. The ECS "External adjustment input" registers (14318-14325) describe three options — `0: Zones, 1: Climate system, 2: Profile` — while declaring `maxValue: 1`. `detect_entity_type()` classifies them as `select` and `parse_description_mapping()` yields all three, so the published entity offers an option the register will refuse.

This is deliberately left alone rather than filtered against the declared range. NIBE's range metadata is demonstrably not authoritative — 4.13.12 alone moved three `maxValue`s and dropped one `minValue` tenfold — so a description enumerating a third option is at least as likely to be right as the bound that excludes it. The two failure modes are not symmetrical: dropping a valid option silently removes a function the user has no way to reach, whereas keeping an invalid one fails safely, since the controller rejects the write and `optimistic: false` leaves the entity showing its real value.

**Per-point override tables (`UNIT_OVERRIDES`, `DIVISOR_OVERRIDES`, `DEVICE_CLASS_OVERRIDES`):** the firmware's own self-reported metadata is trusted by default, but a handful of confirmed exceptions need correcting per-point. `UNIT_OVERRIDES` corrects only a displayed unit label (e.g. a switch firmware wrongly declares `unit: '%'` on); `DIVISOR_OVERRIDES` (paired with `apply_divisor_override()`) corrects the actual scale of a value, for the rarer case where the label is right but the number itself is wrong by a fixed factor — since this changes real math rather than just a string, it must be applied at every point a fresh metadata dict enters the system (`EntityManager._index_point` at discovery, `EntityManager._fetch_bulk_data` every poll), not just once downstream. See [`docs/known-firmware-quirks.md`](https://github.com/whatsinabyte/nibe-smo-mqtt-bridge/blob/main/docs/known-firmware-quirks.md) for the full catalog of *why* each entry exists — evidence, confirmation source, and the GitHub issue/release where it was fixed.
...
**Sentinel value handling (`_process_and_publish_state`):** two distinct forms, both resulting in `offline` (unavailable in HA) rather than a misleading state value, and both entirely separate from the absence/removal handling above — the point is present in the bulk response either way, just not carrying a trustworthy current value. The first is a type-extreme sentinel (`-32768`/`65535`/`-2147483648`/`4294967295` for `s16`/`u16`/`s32`/`u32`) meaning "sensor not connected" — applies to every entity type, completely silent (no log), by original design: it's extremely common (any disconnected accessory sensor hits it) and always means the same understood thing. The second, added when firmware-declared min/max bounds turned out not to be a formality (an unconfigured zone's "desired room temperature" reports `0` outside its own declared 5.0-35.0 °C range, which made HA's MQTT `number` platform reject the value and log a warning every poll), is scoped to `number` entities only — minValue/maxValue doesn't reliably bound other entity types (e.g. a `time` register's natural range routinely exceeds whatever small metadata range it happens to declare). `0` is treated as this firmware's own recognised "unconfigured" convention and stays silent; any other out-of-range value doesn't match a known convention and logs one warning per `point_id` (deduplicated via `_range_warnings_issued`, the same set `nibe_discovery_config.py`'s own one-time build-time diagnostic uses) — worth a human's attention rather than assumed understood.
...
**Nibe Menus dashboard:** rebuilt from scratch on every restart (and on-demand via "Regenerate Dashboard"). Mirrors the full SMO S40 installer menu structure from `menu_structure.yaml` (186 menus, ~1,063 settings). Dynamic points are injected below their controlling switch when active and removed when inactive. Only ever built when `mode == 'menus'` — `EntityManager._publish_dynamic_changes()`'s HA notification checks the currently applied mode (`_read_applied_mode_from_file() == 'menus'`) before wording itself around this dashboard, falling back to pointing at the Nibe Bridge dashboard (provisioned in every mode) otherwise.

**Split menus (`split_submenus`) and subviews:** a top-level menu tagged `split_submenus: true` in `menu_structure.yaml` — currently menus 1, 4, and 7, plus submenu 3.1 specifically rather than its parent menu 3 — doesn't get flattened into one dashboard view. Instead `_build_view_specs()` turns it into a small "hub" view (its own description/warning/note/tip, plus a list of links to its own children — `_build_hub_children_card()`) and one further view per immediate submenu, each rendered fully on its own. The flag isn't limited to top-level menus: `_find_split_descendants()` walks the rest of the tree looking for it on any submenu, letting one oversized branch (e.g. menu 3.1, "Operating info") be pulled out into its own view even though its parent (menu 3) isn't otherwise split — `_render_section()` leaves a short "jump to its own view" card (`_build_jump_out_card()`) where that submenu would have appeared inline. Every view produced this way except the hub itself is marked `"subview": true` in the Lovelace config (requires HA 2024.8+) — it's left out of the dashboard's top tab strip and reached only via the hub's links, showing HA's native back-arrow in its header instead. This exists because flattening a menu with hundreds of settings across a dozen submenus into one tab made that tab unusably long; it also means a cross-reference between two submenus of a split menu, written as "menu X.Y" in another submenu's text, becomes a real clickable link instead of same-page dead text (`_linkify_menu_refs()`/`_resolve_view_id()` resolve to the most specific known view id, not just the top-level one).
