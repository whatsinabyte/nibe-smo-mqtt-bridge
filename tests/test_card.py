...
class TestEscapeHelper(unittest.TestCase):
    ...
    def test_script_tag_escaped(self):
        self.assertEqual(self._esc('<script>alert(1)</script>'),
    ...
    def test_quotes_escaped(self):
        self.assertEqual(self._esc('''say "hi" it's fine'''),
                          'say &quot;hi&quot; it&#39;s fine')
...
class TestEnableDisableToastLogic(unittest.TestCase):
    ...
    def _toast_outcome(self, succeeded, total):
        result = _call_static(self.ctx, '_formatBulkOutcomeToast',
                               'Enabled', 'enable', succeeded, total)
