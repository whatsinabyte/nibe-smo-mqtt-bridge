...
class TestParseCommandPayload(unittest.TestCase):
    ...
    def test_number_degenerate_range_skips_validation(self):
        ei = self._ei(
            "number",
            metadata={"divisor": 1, "minValue": 0, "maxValue": 0},
    ...
    def test_text_non_printable_stripped(self):
        self.assertEqual(
            self.em._parse_command_payload("He\x00llo\x07", self._ei("text"), "t"), "Hello"
        )
    ...
    def test_text_truncated(self):
        ...
        self.assertEqual(
            len(self.em._parse_command_payload(long_s, self._ei("text"), "t")),
    ...
    def test_text_exact_max_not_truncated(self):
        ...
        self.assertEqual(
            len(self.em._parse_command_payload(exact, self._ei("text"), "t")),
            _TEXT_REGISTER_MAX_LEN,
        )
...
class TestParseCommandPayloadProperties(unittest.TestCase):
    ...
    @given(
        st.text(max_size=20).filter(
            lambda s: (
                not (len(s.split(":")) >= 2 and all(p.strip().isdigit() for p in s.split(":")[:2]))
            )
        )
    )
    ...
    def test_unknown_entity_type_always_none(self, payload):
        ...
        result = self.em._parse_command_payload(payload, self._ei("unknown_type_xyz"), "h")
        self.assertIsNone(result)
...
class TestPendingWriteGuardProperties(unittest.TestCase):
    ...
    @given(st.integers(min_value=1, max_value=9999), st.integers(min_value=0, max_value=100))
    def test_matching_value_clears_pending(self, pid, written_value):
        ...
        if em.pending_writes.get(pid, {}).get("value") == em.bulk_data[pid]["raw_value"]:
    ...
    def test_stale_entry_eviction_clears_pid(self, pid):
        ...
        em.pending_writes[pid] = {
            "value": 42,
            "time": _time.time() - _STALE_WRITE_AGE_S - 1,
            "entity_id": f"sensor.nibe_{pid}",
        }
        # Simulate stale eviction
        now = _time.time()
        stale = [p for p, v in em.pending_writes.items() if now - v["time"] > _STALE_WRITE_AGE_S]
        for p in stale:
...
class TestHandleCommand(unittest.TestCase):
    ...
    def test_unexpected_exception_in_handle_command_is_caught_and_logged(self):
        ...
        self.assertTrue(any("Unhandled exception handling command" in msg for msg in cm.output))
    ...
    def test_submit_write_worker_exception_is_caught_and_logged(self):
        ...
        self.assertTrue(
            any("Unhandled exception in write command handler" in msg for msg in cm.output)
        )
    ...
    def test_submitted_cmd_id_matches_the_generated_correlation_token(self):
        ...
        em._write_executor.submit = lambda fn, *a, **kw: fn(*a, **kw)
        with (
            patch.object(em, "_handle_command_worker") as mock_worker,
...
class TestHandleCommandWorkerFailurePath(unittest.TestCase):
    ...
    def test_notification_includes_point_title_and_value(self):
        ...
        em._handle_command_worker(
            self._entity_info(point_id=100, display_title="Permit heating"),
            1,
            "ON",
            "cmd1",
        )
    ...
    def test_successful_write_does_not_touch_failure_counters(self):
        ...
        before = em._write_failed
        em._handle_command_worker(
            self._entity_info(point_id=999, entity_type="button"), 1, "1", "cmd1"
        )
...
class TestForceReadback(unittest.TestCase):
    ...
    def test_readback_uses_force_true(self):
        ...
        with patch.object(em, "_process_and_publish_state") as mock_process:
            em._force_readback(self._entity_info(point_id=100))
        self.assertTrue(
            mock_process.call_args.kwargs.get("force")
...
class TestHandleCommandWorkerWriteSuccessTime(unittest.TestCase):
    ...
    def test_time_entity_optimistic_state_hhmmss(self):
        ...
        state_calls = [c for c in em.mqtt.publish.call_args_list if c.args[0] == "nibe/state/300"]
    ...
    def test_time_seconds_wrap_at_86400_not_86401(self):
        ...
        state_calls = [c for c in em.mqtt.publish.call_args_list if c.args[0] == "nibe/state/301"]
...
class TestHandleCommandWorkerOptimisticStateBranches(unittest.TestCase):
    ...
    def test_case_a1_non_controlling_fully_processed_opens_no_scan(self):
        ...
        with (
            patch.object(em, "_open_post_write_scan") as mock_scan,
...
class TestWritePointLastStatesRepublishOnUnderflow(unittest.TestCase):
    ...
    def test_last_states_republished_on_underflow(self):
        ...
        self.assertIsNone(result)
        calls = [c for c in em.mqtt.publish.call_args_list if c.args[0] == "nibe/state/200"]
