    ...
    def getitem_dict_access(self, key):
        ...
        if present:
            ...
            assert value == readback, (
                f"cache[{key}]={value!r} but cache.get({key})={readback!r}"
            )
            ...
            try:
                ...
                raise AssertionError(
                    f"cache[{key}] should have raised KeyError for an absent key"
                )
            stats_after = self.cache.get_stats()
...
class ValueCacheMachine(RuleBasedStateMachine):
    ...
    def last_publish_keys_match_model(self):
        ...
        assert publish_keys == self.model_last_publish_keys, (
            f"_last_publish keys {publish_keys} != expected "
            f"{self.model_last_publish_keys}"
        )
    ...
    def last_publish_keys_subset_of_cache_keys(self):
        ...
        assert publish_keys <= cache_keys, (
            f"_last_publish keys {publish_keys} not a subset of "
            f"_cache keys {cache_keys}"
        )
