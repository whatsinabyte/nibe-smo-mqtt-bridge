...
class TestRunPing(unittest.TestCase):
    ...
    def test_other_returncode_includes_stderr_detail(self):
        ...
        with patch(
            "subprocess.run",
            return_value=MagicMock(returncode=2, stdout="", stderr="ping: bad address"),
        ):
    ...
    def test_ping_timeout_expired(self):
        ...
        with patch("subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="ping", timeout=20)):
            result = _run_ping("192.0.2.1")
        self.assertFalse(result["ok"])
...
class TestRunCurl(unittest.TestCase):
    ...
    def test_credentials_rejected_exact_dict(self):
        ...
        with patch("subprocess.run", return_value=self._curl_result(stdout="HTTP_CODE:401")):
            result = _run_curl(
                "https://192.0.2.1:8443/api/v1/devices/0", "/ssl/nibe-ca.pem", "Basic wrong="
            )
        self.assertEqual(
            result,
            {
...
class TestRunCurlProperties(unittest.TestCase):
    ...
    def test_any_2xx_with_auth_header_is_ok(self, http_code):
        ...
        with patch(
            "subprocess.run", return_value=self._curl_result(stdout=f"HTTP_CODE:{http_code}")
        ):
            result = _run_curl(
                "https://192.0.2.1:8443/api/v1/devices/0",
                None,
    ...
    @given(
        http_code=st.integers(min_value=300, max_value=999).filter(lambda c: c not in (401, 403))
    )
...
class TestRunConnectivityCheck(unittest.TestCase):
    ...
    def _patch_both(self, ping_ok, curl_ok):
        return (
            patch(
                "nibe_connectivity_check._run_ping",
                return_value={"ok": ping_ok, "summary": "ping result"},
            ),
            patch(
                "nibe_connectivity_check._run_curl",
                return_value={
                    "ok": curl_ok,
                    "summary": "curl result",
                    "http_code": 200 if curl_ok else None,
                },
            ),
        )
    ...
    def test_ping_and_curl_both_fail_but_curl_got_an_http_response_is_not_reported_as_full_outage(
        self,
    ):
    ...
    def test_ping_ok_curl_got_error_status_reports_curl_summary_not_generic_message(self):
        ...
        with (
            patch("nibe_connectivity_check._run_ping", return_value={"ok": True, "summary": "x"}),
            patch(
                "nibe_connectivity_check._run_curl",
                return_value={
                    "ok": False,
    ...
    def test_ping_invoked_with_the_real_host(self):
        ...
        with (
            patch(
                "nibe_connectivity_check._run_ping", return_value={"ok": True, "summary": "x"}
            ) as mock_ping,
            patch(
                "nibe_connectivity_check._run_curl",
    ...
    def test_ca_cert_path_and_auth_header_forwarded_to_curl(self):
        ...
        with (
            ...
            patch(
                "nibe_connectivity_check._run_curl",
                return_value={"ok": True, "summary": "x", "http_code": 200},
            ) as mock_curl,
        ):
            run_connectivity_check(
                "192.0.2.1",
                "https://192.0.2.1:8443/api/v1/devices/0",
                ca_cert_path="/ssl/nibe-ca.pem",
                auth_header="Basic dGVzdA==",
            )
