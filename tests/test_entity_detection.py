...
class TestApplyDivisor(unittest.TestCase):
    ...
    def test_divisor_one(self):
        self.assertEqual(self.fn(100, 1), "100")

    def test_divisor_zero_as_one(self):
        self.assertEqual(self.fn(100, 0), "100")

    def test_divisor_ten(self):
        self.assertEqual(self.fn(348, 10), "34.8")

    def test_trailing_zero_stripped(self):
        self.assertEqual(self.fn(350, 10), "35")

    def test_divisor_hundred(self):
        self.assertEqual(self.fn(1234, 100), "12.34")

    def test_zero_raw(self):
        self.assertEqual(self.fn(0, 10), "0")

    def test_negative_raw(self):
        self.assertEqual(self.fn(-50, 10), "-5")

    def test_large_divisor(self):
        self.assertEqual(self.fn(12345, 1000), "12.345")
...
class TestReverseDivisor(unittest.TestCase):
    ...
    def test_divisor_one(self):
        self.assertEqual(self.fn(42.0, 1), 42)

    def test_divisor_zero_as_one(self):
        self.assertEqual(self.fn(42.0, 0), 42)

    def test_divisor_ten(self):
        self.assertEqual(self.fn(34.8, 10), 348)

    def test_negative(self):
        self.assertEqual(self.fn(-5.0, 10), -50)

    def test_zero(self):
        self.assertEqual(self.fn(0.0, 10), 0)
    ...
    def test_round_trip(self):
        ...
        for raw in [150, 200, 250, 300, 348, 400]:
            ...
            self.assertEqual(self.fn(display, 10), raw, f"round-trip failed for raw={raw}")
...
class TestCleanString(unittest.TestCase):
    ...
    def test_normal(self):
        self.assertEqual(self.fn("Supply temperature"), "Supply temperature")
    ...
    def test_mojibake_c2(self):
        self.assertEqual(self.fn("a\u00c2b"), "ab")
...
class TestApplyDivisorProperties(unittest.TestCase):
    ...
    @example(
        raw_value=360, divisor=60
    )  # 6.0 min — point 1024 (Timer EME), only divisor=60 in firmware
    ...
    @given(
        st.floats(min_value=-32768.0, max_value=32767.0, allow_nan=False, allow_infinity=False),
    ...
    @given(st.floats(min_value=-32768.0, max_value=32767.0, allow_nan=False, allow_infinity=False))
    def test_reverse_divisor_zero_same_as_one(self, display_value):
        ...
        self.assertEqual(reverse_divisor(display_value, 0), reverse_divisor(display_value, 1))


class TestReverseDivisorProperties(unittest.TestCase):
    """Hypothesis properties for reverse_divisor."""

    @given(st.integers(min_value=-32768, max_value=32767), st.integers(min_value=1, max_value=1000))
...
class TestApplyDivisorExtendedProperties(unittest.TestCase):
    ...
    def test_no_trailing_zeros_after_decimal(self, raw_value, divisor):
        ...
        if "." in result:
            ...
            self.assertFalse(result.endswith("."), f"Trailing decimal in {result!r}")
...
class TestParseDescriptionMappingProperties(unittest.TestCase):
    ...
    _label = (
        ...
        .map(str.strip)
        .filter(lambda s: s and not s.lstrip("-").isdigit())
    )

    @given(
        st.dictionaries(
            st.integers(min_value=-1000, max_value=1000), _label, min_size=1, max_size=6
        )
    )
...
class TestDetectEntityTypeProperties(unittest.TestCase):
    ...
    @given(
        _nibe_point_id,
        st.sampled_from(
            ["MODBUS_INPUT_REGISTER", "MODBUS_HOLDING_REGISTER", "MODBUS_NO_REGISTER", ""]
        ),
...
class TestGetEntityOptionsProperties(unittest.TestCase):
    ...
    @given(
        st.lists(st.integers(min_value=-1000, max_value=1000), min_size=2, max_size=6, unique=True),
    ...
    def test_agrees_with_parse_description_mapping_int_left(self, keys, labels):
        ...
        mapping = dict(zip(keys[:n], labels[:n], strict=False))
        description = ", ".join(f"{k} = {v}" for k, v in mapping.items())
...
class TestMapDeviceClassProperties(unittest.TestCase):
    ...
    @given(
        st.text(max_size=20).filter(lambda s: s not in ("sensor", "binary_sensor", "number")),
        st.text(max_size=20),
        st.text(max_size=80),
    )
...
class TestModesStructuralProperties(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._sized = {k: v for k, v in MODES.items() if v is not None and len(v) > 0}
    ...
    def test_mode_points_are_all_ints(self):
        ...
        for name, pts in self.MODES.items():
            if pts is not None:
                for pid in pts:
                    self.assertIsInstance(pid, int, f"MODES[{name!r}] contains non-int: {pid!r}")
    ...
    def test_mode_point_ids_in_valid_range(self):
        ...
        for name, pts in self.MODES.items():
            if pts is not None:
                for pid in pts:
                    self.assertGreater(pid, 0, f"MODES[{name!r}] contains zero/negative pid: {pid}")
                    self.assertLessEqual(
                        pid, 65535, f"MODES[{name!r}] contains out-of-range pid: {pid}"
                    )
...
class TestDetectHoldingEntityProperties(unittest.TestCase):
    ...
    def _point(self, pid, var_type="integer", var_size="s16", description="", writable=True):
        return {
            "variableId": pid,
...
class TestGetRegisterType(unittest.TestCase):
    ...
    def test_holding(self):
        self.assertEqual(
            self.fn({"metadata": {"modbusRegisterType": "MODBUS_HOLDING_REGISTER"}}), "holding"
        )

    def test_input(self):
        self.assertEqual(
            self.fn({"metadata": {"modbusRegisterType": "MODBUS_INPUT_REGISTER"}}), "input"
        )

    def test_unknown(self):
        self.assertIsNone(self.fn({"metadata": {"modbusRegisterType": "X"}}))

    def test_no_metadata(self):
        self.assertIsNone(self.fn({}))

    def test_empty_meta(self):
        self.assertIsNone(self.fn({"metadata": {}}))
...
class TestParseDescriptionMapping(unittest.TestCase):
    ...
    def test_int_left(self):
        self.assertEqual(self.fn("0 = Off, 1 = Active"), {0: "Off", 1: "Active"})

    def test_label_left(self):
        self.assertEqual(self.fn("Auto = 0, Manual = 1"), {0: "Auto", 1: "Manual"})

    def test_no_equals(self):
        self.assertIsNone(self.fn("Off Active"))
    ...
    def test_single_pair(self):
        self.assertEqual(self.fn("0 = Off"), {0: "Off"})

    def test_whitespace(self):
        self.assertEqual(self.fn("  0  =  Off  ,  1  =  On  "), {0: "Off", 1: "On"})
...
class TestApiSpecConformance(unittest.TestCase):
    ...
    def test_modbus_no_register_returns_sensor(self):
        ...
        entity_type, category = self.detect_type(self._point("MODBUS_NO_REGISTER"))
    ...
    def test_get_register_type_no_register_returns_none(self):
        ...
        result = self.get_reg_type({"metadata": {"modbusRegisterType": "MODBUS_NO_REGISTER"}})
        self.assertIsNone(result)

    def test_get_register_type_err_unknown_returns_none(self):
        result = self.get_reg_type({"metadata": {"modbusRegisterType": "ERR_UNKNOWN"}})
        self.assertIsNone(result)
    ...
    def test_variabletype_binary_holding_classified(self):
        ...
        entity_type, _ = self.detect_type(
            self._point("MODBUS_HOLDING_REGISTER", var_type="binary", var_size="u8", writable=True)
        )
        self.assertIsInstance(entity_type, str)
    ...
    def test_variabletype_binary_input_classified(self):
        entity_type, _ = self.detect_type(
            self._point("MODBUS_INPUT_REGISTER", var_type="binary", var_size="u8", writable=False)
        )
        self.assertIsInstance(entity_type, str)

    def test_variabletype_unknown_does_not_crash(self):
        """variableType=unknown is explicitly in the spec enum."""
        entity_type, _ = self.detect_type(self._point("MODBUS_INPUT_REGISTER", var_type="unknown"))
        self.assertIsInstance(entity_type, str)
    ...
    def test_variabletype_floating_point_holding(self):
        ...
        entity_type, _ = self.detect_type(
            self._point(
                "MODBUS_HOLDING_REGISTER", var_type="floating-point", var_size="f4", writable=True
            )
        )
        self.assertIsInstance(entity_type, str)
    ...
    def test_variablesize_f4_does_not_crash(self):
        entity_type, _ = self.detect_type(
            self._point("MODBUS_INPUT_REGISTER", var_type="floating-point", var_size="f4")
        )
        self.assertIsInstance(entity_type, str)

    def test_variablesize_f8_does_not_crash(self):
        entity_type, _ = self.detect_type(
            self._point("MODBUS_INPUT_REGISTER", var_type="floating-point", var_size="f8")
        )
        self.assertIsInstance(entity_type, str)
    ...
    def test_variabletype_string_holding_maps_to_text(self):
        ...
        entity_type, category = self.detect_type(
            self._point("MODBUS_HOLDING_REGISTER", var_type="string", writable=True)
        )
    ...
    def test_variabletype_string_input_maps_to_sensor(self):
        ...
        entity_type, category = self.detect_type(
            self._point("MODBUS_INPUT_REGISTER", var_type="string", writable=False)
        )
        self.assertEqual(entity_type, "sensor")
        self.assertEqual(category, "diagnostic")
    ...
    def test_variabletype_string_logs_debug(self):
        ...
        with self.assertLogs("nibe.detection", level="DEBUG") as cm:
            self.detect_type(
                self._point(
                    "MODBUS_HOLDING_REGISTER", var_type="string", writable=True, point_id=8001
                )
            )
        self.assertTrue(any("text" in m.lower() or "string" in m.lower() for m in cm.output))
    ...
    def test_variabletype_floating_point_logs_debug(self):
        ...
        with self.assertLogs("nibe.detection", level="DEBUG") as cm:
            self.detect_type(
                self._point(
                    "MODBUS_INPUT_REGISTER", var_type="floating-point", var_size="f4", point_id=8002
                )
            )
    ...
    def test_variabletype_time_input(self):
        ...
        entity_type, _ = self.detect_type(self._point("MODBUS_INPUT_REGISTER", var_type="time"))
        self.assertEqual(entity_type, "sensor")
...
class TestDetectEntityTypeWarningLogging(unittest.TestCase):
    ...
    def test_known_override_point_5110_logs_correct_auto_type(self):
        ...
        point = self._point(
            5110,
            "Prevent condensation climate system 1",
...
class TestNibeApiHttpErrorPaths(unittest.TestCase):
    ...
    def test_write_device_mode_http_400_returns_false(self):
        with patch("urllib.request.urlopen", side_effect=self._http_error(400, b"bad value")):
...
class TestLastStatesFallbackPublishProperties(unittest.TestCase):
    ...
    def test_absent_last_state_always_publishes(self, raw, divisor):
        ...
        state_calls = [
            c for c in em.mqtt.publish.call_args_list if c.args[0] == f"nibe/state/{self._PID}"
        ]
        self.assertTrue(state_calls)

    @given(
        raw=st.integers(min_value=-1000, max_value=1000),
        stale=st.text(min_size=1, max_size=20, alphabet=st.characters(categories=["L", "N", "P"])),
        divisor=st.sampled_from([1, 10, 100]),
    )
...
class TestDetectInputEntityDescriptionParsing(unittest.TestCase):
    ...
    def test_description_with_both_equals_and_comma_gives_select(self):
        ...
        p = self._holding_point(description="0=Stop,1=Low,2=High", min_val=0, max_val=2)
        entity_type, _ = _detect_holding_entity(p, p["metadata"])
        self.assertEqual(entity_type, "select")
...
class TestDetectInputEntityVarTypeStrings(unittest.TestCase):
    ...
    def _point(
        self, var_type, var_size="u16", point_id=99002, min_val=0, max_val=100, description=""
    ):
        return {
            "variableId": point_id,
            "display_title": "Test",
