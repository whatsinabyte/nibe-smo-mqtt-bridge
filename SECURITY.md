...
This app communicates with your Nibe controller over your local network using
...
- **Credentials** — your Nibe API username and password are stored in the
  Home Assistant app configuration. Protect access to your HA instance
  accordingly. As an alternative, credentials can be supplied as a pre-encoded
  Basic auth token in `secrets.yaml` to keep raw credentials out of the app
...
- **Local network** — the app requires host network access to reach your
...
- **MQTT** — if your Mosquitto broker has authentication disabled, any device
  on your local network can publish to the broker topics used by this app.
  Enabling authentication on the broker is strongly recommended. When credentials
  are configured but `mqtt_tls` is not enabled, the app logs a warning at
...
- **TLS — Nibe device API** — the Nibe controller uses a self-signed certificate
  for its local API. By default the app accepts this certificate without
  ...
  in the app options — full chain verification and hostname checking will then
...
- **TLS — MQTT broker** — by default, MQTT traffic (including credentials and
  ...
  Set `mqtt_tls: true` in the app options to encrypt broker traffic. If your
  ...
  Mosquitto app bundled with Home Assistant supports TLS on port 8883.
- **HA Supervisor API** — the app uses the HA Supervisor REST API and
